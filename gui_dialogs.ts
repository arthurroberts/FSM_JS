...
function make_new_state_dlg(point_to_inser:Point2D){
    ...
    if (element){   // svg anchor exists. Is it needed - dialogs are not
        ...
        let myElement = document.getElementById("new_state_dlg_id") as HTMLElement;
        if (myElement) { myElement.remove();}

        let new_state_dlg = document.createElement("dialog") as HTMLDialogElement;//document.createElement("dialog") as HTMLDialogElement;
        new_state_dlg.id = 'new_state_dlg_id';

        //let title = document.createTextNode("New state name");

        let new_state_name_txt = document.createElement("input") as HTMLInputElement;
        ...
        let state_label = document.createElement("label") as HTMLLabelElement;
        state_label.setAttribute("for", "new_state_name_txt");
        state_label.textContent = "State name:";

        let new_state_out_txt = document.createElement("input") as HTMLInputElement;
        ...
        let out_label = document.createElement("label") as HTMLLabelElement;
        out_label.setAttribute("for", "new_state_out_txt");
        out_label.textContent = "Output:";

        let okButton = document.createElement("button") as HTMLButtonElement;
        okButton.setAttribute("id", "okButton"); okButton.innerHTML = "OK";
        let cancelButton = document.createElement("button") as HTMLButtonElement;
        cancelButton.setAttribute("id", "cancelButton"); cancelButton.innerHTML = "Cancel";
        let myBr = document.createElement("br") as HTMLBRElement;

        // Add an event listener to the buttons
        okButton.addEventListener("click", () => { // OK clicked
            ...
            if (new_name) {fsm.add_state(new_name,point_to_inser,new_output);
                let new_st = fsm.find_state_by_name(new_name);
                if (new_st!== null) new_st.output = new_output;
                draw_fsm();
                gui_mode = 0;
                new_state_dlg.close();
             }
        });
        cancelButton.addEventListener("click", () => { new_state_dlg.close();
             gui_mode = 0; });
...
function make_edit_state_dlg(sel_state:string){
   
  let element = document.getElementById('edit_state_dlg_anchor');// find anchor
  if (element){  
    // delete if exists
    let myElement = document.getElementById("edit_state_dlg_id") as HTMLElement;
    if (myElement) { myElement.remove();}

    let edit_state_dlg = document.createElement("dialog") as HTMLDialogElement;
    edit_state_dlg.id = 'edit_state_dlg_id';
    // state name text box
    let state_name_txt = document.createElement("input") as HTMLInputElement;
    ...
    let state_out_txt = document.createElement("input") as HTMLInputElement;
    state_out_txt.setAttribute("id", "myInput");
    state_out_txt.setAttribute("type", "text");
    state_out_txt.setAttribute("size", "5");
    let outp = fsm.find_state_by_name(sel_state)?.output;
    if (outp!==undefined)    state_out_txt.value = outp;
    ...
    deleteButton.setAttribute("id", "deleteButton"); deleteButton.innerHTML = "Delete state";
    deleteButton.addEventListener("click", () => { 
        let state_to_delete_name:string = ' ';
    ...
    //move state button
    let moveButton = document.createElement("button") as HTMLButtonElement;
    moveButton.setAttribute("id", "moveButton"); moveButton.innerHTML = "Move state";
    moveButton.addEventListener("click", () => { 
        console.log(' moveButton click');
        gui_mode = 11;
    });
    //make it accepting state button
    let accepButton = document.createElement("button") as HTMLButtonElement;
    accepButton.setAttribute("id", "moveButton"); accepButton.innerHTML = "Make accepting";
    ...
    let initButton = document.createElement("button") as HTMLButtonElement;
    initButton.setAttribute("id", "moveButton"); initButton.innerHTML = "Make initial";
...
function make_new_tran_dlg(selected_state_name:string){
    
 let element = document.getElementById('new_tran_dlg_anchor');

 if (element){  
    let myElement = document.getElementById("new_tran_dlg_id") as HTMLElement;
    if (myElement) { myElement.remove();}

    var new_tran_dlg = document.createElement("dialog") as HTMLDialogElement;
    new_tran_dlg.id = 'new_tran_dlg_id';

    let sou_state_name_txt = document.createElement("input") as HTMLInputElement;
    ...
    let sou_label = document.createElement("label") as HTMLLabelElement;
    sou_label.setAttribute("for", "mySou");
    sou_label.textContent = "Source:";

    let input_txt = document.createElement("input") as HTMLInputElement;
    input_txt.setAttribute("id", "myInput");
    input_txt.setAttribute("type", "text");
    input_txt.setAttribute("size","5");
    input_txt.value = '';
    let input_label = document.createElement("label") as HTMLLabelElement;
    input_label.setAttribute("for", "myInput");
    input_label.textContent = "Input signal:";

    let dest_state_name_txt = document.createElement("input") as HTMLInputElement;
    ...
    let dest_label = document.createElement("label") as HTMLLabelElement;
    dest_label.setAttribute("for", "dest");
    dest_label.textContent = "Destination:";

    let out_mealy = document.createElement("input") as HTMLInputElement;
    ...
    okButton.addEventListener("click", () => { 
        ...
        let new_tr = new Transition( 
              removeWhitespace( input_txt.value),dest_state_name_txt.value,
                                sou_state_name_txt.value,out_mealy.value );

        let p1=new Point2D(0,0);
        let p1_pos = fsm.find_state_by_name(sou_state_name_txt.value)?.position; 
        if (p1_pos!==undefined){ p1 = p1_pos;}
        let p2=new Point2D(0,0);
        let p2_pos = fsm.find_state_by_name(dest_state_name_txt.value)?.position; 
        if (p2_pos!==undefined){ p2 = p2_pos;}
...
function make_edit_tran_dlg(sel_tran:Transition){
 let element = document.getElementById('new_tran_dlg_anchor');
 if (element){  // anchor exists
    let myElement = document.getElementById("edit_tran_dlg_id") as HTMLElement;
    if (myElement) { myElement.remove();}

    var edit_tran_dlg = document.createElement("dialog") as HTMLDialogElement;
    ...
    let output_m = document.createElement("input") as HTMLInputElement;
    ...
    let output_m_label = document.createElement("label") as HTMLLabelElement;
    ...
    cancelButton.setAttribute("id", "cancelButton"); cancelButton.innerHTML = "Cancel";
    let deleteButton = document.createElement("button") as HTMLButtonElement;
    deleteButton.setAttribute("id", "okButton"); deleteButton.innerHTML = "Delete";
    let moveButton = document.createElement("button") as HTMLButtonElement;
    moveButton.setAttribute("id", "moveButton"); moveButton.innerHTML = "Move";
    let okButton = document.createElement("button") as HTMLButtonElement;
    okButton.setAttribute("id", "okButton"); okButton.innerHTML = "OK";
    ...
    okButton.addEventListener("click", () => { 
        ...
        let p1 = fsm.find_state_by_name(sou_state_name_txt.value)?.position;
        let p2 = fsm.find_state_by_name(dest_state_name_txt.value)?.position;
        //let int = 
        if ((p1) && (p2)){
