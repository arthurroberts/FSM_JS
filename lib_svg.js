...
function dist_point_line(p1, p2, p3) {
    let dx21 = p2.x - p1.x;
    let dy21 = p2.y - p1.y;
    let dx31 = p3.x - p1.x;
    let dy31 = p3.y - p1.y;
...
function circle_from_points(p1, p2, p3, c) {
...
class Arc {
    //this_id:string;
    constructor(p1in, p2in, id_in, label) {
        this.p1 = p1in;
        this.p2 = p2in;
        this.is_line = true;
        ...
        this.sweepflag = 0;
        this.large_small = 1;
        this.midpoint = new Point2D((this.p1.x + this.p2.x) / 2, (this.p1.y + this.p2.y) / 2);
        this.id = " ";
        this.label = label;
    }
...
class SVG {
    ...
    changeArrowById(p_new, angle_new, id) {
    ...
    drawLine(p1, p2, red, green, blue, id, vis) {
    ...
    drawCircle(cent, r, sred, sgreen, sblue, fred, fgreen, fblue) {
    ...
    */
    drawRect(xc, yc, h, w, s_r, s_g, s_b, f_r, f_g, f_b) {
  ...
  */
    drawSurface(x0, x1, dx, y0, y1, dy, z_arr) {
        //let svg_width  = this.svg.width;
        //let svg_height = this.svg.height;
        let svg_width = Number(this.svg.getAttribute('width'));
        let svg_height = Number(this.svg.getAttribute('height'));
        let margin = 30;
