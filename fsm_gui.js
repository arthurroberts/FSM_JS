...
var selected_transition;
// drawing part - create svg area
var svg_width = 900;
var svg_height = 1200;
var svg_fsm = new SVG(svg_width, svg_height);
(_a = document.getElementById('svg_anchor')) === null || _a === void 0 ? void 0 : _a.appendChild(svg_fsm.svg);
...
function load_server_local(file_name) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log('loading file ' + file_name);
        let response = yield fetch('./' + file_name);
...
function save_fsm() {
    ...
    let svg_str = '<?xml version="1.0" standalone="no"?>';
...
function new_fsm() {
    ...
    svg_fsm.clear();
    // clear interface
    let element = document.getElementById('input_seq');
    element.value = '';
    element = document.getElementById('output_seq');
    element.value = '';
    draw_fsm();
}
/**MOUSE PROCESSING */
...
function ms_click(event) {
    switch (gui_mode) {
        ...
        case 2:
            ...
            if (selected_transition !== null)
                change_transition(selected_transition, click_xy);
            svg_fsm.clear();
            draw_fsm();
            gui_mode = 0;
            break;
...
function find_state_clicked(clickp) {
...
function draw_fsm() {
    ...
    fsm.states.forEach((state) => {
        if (state.name == fsm.initial_state) {
            svg_fsm.drawLine(new Point2D(state.position.x - 50, state.position.y), state.position, 150, 120, 120, 'ini', true);
        }
        ...
        // drw state name  
        svg_fsm.drawText(new Point2D(state.position.x - r * 0.9, state.position.y + 0.2 * r), state.get_label());
    });
    ...
    if (ini_st) {
        if ((ini_st) && (ini_st.output != ' ')) {
            ...
            let outdisp = document.getElementById("output_seq");
            if (outdisp) {
                if (outdisp.value === '')
                    outdisp.value = ini_st.output;
            }
            //document.getElementById("output_seq").value = 
        }
    }
}
function change_state_position(state_name_to_move, new_position) {
    ...
    fsm.states.forEach((state) => {
        ...
        state.transitions.forEach((tran) => {
            ...
            else if (tran.destination == state_name_to_move) { //incoming arc
            ...
            else if (tran.source == state_name_to_move) { // outgoing arc
...
function change_transition(selected_transition, clickp) {
    console.log(' moving_transition ' + selected_transition);
    selected_transition.arc.setArcBy3Points(selected_transition.arc.p1, selected_transition.arc.p2, clickp);
}
