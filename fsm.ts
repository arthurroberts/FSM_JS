...
class Transition{
    signal:string;  // can be an array of strings
    destination:string;
    source:string;
    output:string; // Mealy machine
    arc:Arc;
    constructor(sig:string,dest:string,sou:string,out:string){
       this.signal = sig;
    ...
    set(sig:string,dest:string,sou:string){
        this.signal = sig;
        this.destination = dest;
        this.source = sou;
    ...
    get_label():string{
        let trimmed_str_length:number = this.output.trim().length;
...
class State{
    ...
    output:string; //Moore machine
    position:Point2D; //graphical

    constructor(n:string){
        this.name = n;
        this.transitions = new Array<Transition>( ); // maps input signal to transition
        this.position = new Point2D(0,0);
        this.output = ' ';
    }

    add_transition(tr:Transition){
        let p1:Point2D = this.position;
        let dest_state = fsm.find_state_by_name(tr.destination);
        if (dest_state != undefined){
          let p2:Point2D = dest_state.position;
          tr.arc.p1 = p1;
          tr.arc.p2 = p2;
...
class FSM{
    //self;
    states: Array <State>;   // Map - maps state name to State class
    initial_state:string;
    current_state:string; // for NFA can be several of them
    accepting_states:string[]; // can be several accepting states
    current_output:string; //for Mealy
    current_state_output:string; //for Moore
    ...
    add_state(name:string, pos:Point2D,output:string) {
        let new_state:State = new State(name);
    ...
    delete_state(name:string){
        ...
        for (let i:number = 0 ; i<this.states.length ; i++){
            for (let j:number = 0 ; j<this.states[i].transitions.length; j++){
    ...
    add_transition(sou:string, tra:Transition, arc:Arc){

        let st1 = this.find_state_by_name(sou);
        let st2 = this.find_state_by_name(tra.destination);
        
        if ((st1)&&(st2)){ // states exist
            let p1end:Point2D = st1.position;
            let p2end:Point2D = st2.position;
            //create arc/line 
            if ( this.find_state_by_name(sou)?.name == 
              this.find_state_by_name(tra.destination)?.name){
    ...
    delete_transition(source_name:string,signal:string){
        let state = this.find_state_by_name(source_name);
        if (state!==null){
          for (let i:number = 0; i < state.transitions.length; i++){
    ...
    find_state_by_name(name:string):State|null{
        for ( let i:number =0 ; i< this.states.length; i++){
            if (this.states[i].name == name){
        ...
        if (cs!==null){
          ...
          if (poss_tran!==null){
              ...
              if (this.current_output != ' '){
                    if (poss_tran.output!==undefined) this.current_output = poss_tran.output;
              }
              //Moore
              let moore = this.find_state_by_name(this.current_state)?.output;
...
function fsm_from_json(data:string){
    fsm.states.length = 0;
 ...
 //   console.log('json_parsed='+json_parsed);
   ...
   // console.log('there are '+json_parsed.states.length+' states');
    ...
    for (let i =0 ; i< json_parsed.states.length; i++) {
        ...
        fsm.add_state(state_name, new Point2D(parseFloat(x),
                                              parseFloat(y)),output);
        //let st = fsm.find                                      
    };
